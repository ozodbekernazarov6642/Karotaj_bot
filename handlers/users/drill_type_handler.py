...
async def send_expedition(call: types.CallbackQuery):
    ...
    await call.message.answer("Burg'u turini tanlang", reply_markup=preparation_time)
...
async def send_expedition(call: types.CallbackQuery):
    app_list["drill_type"] = call.data
    await call.message.answer("Burg'u tayyor bo'lish vaqtini tanlng", reply_markup=preparation_time)
    await call.message.delete()
    await menu_uz.next()
